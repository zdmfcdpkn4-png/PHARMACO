...
export const getWorkspace = asyncHandler(async (req, res) => {
  ...
  // Comme listBoards : les projets archivés sont masqués par défaut
  // (include_archived=true pour tout renvoyer).
  const { include_archived } = req.query;
  const showArchived = include_archived === 'true' || include_archived === '1';
  const boards = await query(
    `SELECT id, name, description, created_at FROM boards
     WHERE workspace_id = $1 ${showArchived ? '' : 'AND archived = false'}
     ORDER BY id`,
    [req.params.id]
  );
